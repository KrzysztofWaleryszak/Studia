...
public class Reactions : MonoBehaviour
{
	...
	{
		_walking._animator.SetInteger("Lvl", var);
		FindObjectOfType<LevelManager>().marioLvl = var;
		_walking._animator.SetTrigger("newLvl");
		if(var==0)
		{
	...
	IEnumerator EnumHurt(int velocity)
	{
		...
		_walking._canMoving = false;
		_walking._rigid.AddForce(new Vector2( velocity*_walking.levelEntryWalkSpeedX,_walking.jumpForce));
		for (int i = 0; i < 3; i++)
		{
